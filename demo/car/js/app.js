...
(function (app) {
    var MAX_OBJECTS = 4;
    var CarDemo = (function (_super) {
        __extends(CarDemo, _super);
        function CarDemo() {
            _super.call(this, "CarDemo");
            this.cars = [];
            this.arrows = [];
            this.areas = [];
            this.objCount = 0;
        }
        CarDemo.prototype.onInitialize = function () {
            var _this = this;
            var _loop_2 = function(i) {
                num = i + 1;
                this_2.cars[i] = this_2.find("car" + num);
                this_2.arrows[i] = this_2.find("arrow" + num);
                this_2.areas[i] = this_2.find("area" + num);
                if (!this_2.cars[i])
                    return "break";
                this_2.initCar(i);
                this_2.objCount++;
                if (this_2.areas[i])
                    this_2.areas[i].visible = false;
                btn_clip = this_2.find("btn" + num);
                if (btn_clip)
                    new fl.Button(btn_clip, function () { return _this.onBtnClick(i); });
            };
            var this_2 = this;
            var num, btn_clip;
        ...
        CarDemo.prototype.onBtnClick = function (num) {
            var area = this.areas[num];
            if (!area)
                return;
            area.visible = true;
            area.color.a = 0;
            createjs.Tween.get(area.color)
                .to({ a: 1 }, 100)
                .wait(1000)
                .to({ a: 0 }, 400)
                .call(function () { return area.visible = false; });
        };
        CarDemo.prototype.initCar = function (num) {
            var _this = this;
            var car = this.cars[num];
            var hl = this.findIn(car, "car_hl");
            hl.color.a = 0;
            hl.buttonMode = true;
            hl.interactive = true;
            var btn = hl;
            btn.on("mouseover", function () { return hl.color.a = 1; });
            btn.on("mouseout", function () { return hl.color.a = 0; });
            btn.on("mouseup", function () { return _this.onCarClick(num); });
            btn.on("touchstart", function () { return hl.color.a = 1; });
            btn.on("touchendoutside", function () { return hl.color.a = 0; });
            btn.on("touchend", function () { return _this.onCarClick(num); });
        };
        CarDemo.prototype.onCarClick = function (num) {
            var car = this.cars[num];
            var hl = this.findIn(car, "car_hl");
            hl.visible = false;
            var arrow = this.arrows[num];
            if (arrow)
                createjs.Tween.get(arrow.color).to({ a: 0 }, 200);
            car.animation.onComplete(function () {
                car.gotoFirstFrame();
                hl.visible = true;
                hl.color.a = 0;
                if (arrow)
                    createjs.Tween.get(arrow.color).to({ a: 1 }, 200);
            });
            car.animation.playFromBeginToEnd();
        };
        CarDemo.prototype.find = function (name) {
            return this.findIn(this.content, name);
        };
        CarDemo.prototype.findIn = function (content, name) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                if (child.name == name)
                    return child;
                if (child.resource != null && child.resource.name == name)
                    return child;
            }
            return null;
        };
...
(function (app) {
    app.WIDTH = 800;
    app.HEIGHT = 600;
    app.STAGE_COLOR = 0xaaaaaa;
    app.FORCE_USE_CANVAS = false;
    app.DEBUG = false;
    var _renderer;
    var _stage;
    var _container;
    ...
    function changeScreen(screen) {
        ...
        _screen = screen;
        _screen.initialize();
        _stage.addChild(_screen.content);
        adjustScreenSize();
    }
